...
// example usage: blogs.php?blog_name=qb
session_start();
// include config file
...
if ( isset($_GET['blog_name']) ) {
  ...
  if ( $b = blog_load($blog_name) ) {
    ...
    $page->BlogMetaData = "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\" />"
      ."<meta name=\"generator\" content=\"Qikblogger\" />"
      ."<link rel=\"alternate\" type=\"application/atom+xml\" title=\"$b->blog_title\" href=\"".strtolower(strtok($_SERVER['SERVER_PROTOCOL'], '/')).'://'.$_SERVER['HTTP_HOST'].'atom.php'."\" />";
    
    // setting the type of page to be displayed
    $page->MainPage = true;
    $page->ArchivePage = false;
    $page->MainOrArchivePage = true;
    $page->ItemPage = false; // setting this to false bcoz this would have multiple blog entries
    
    // setting info related to the Blog
    $page->BlogPageTitle = $b->blog_pgtitle;
    $page->BlogTitle = $b->blog_title;
    $page->BlogURL = $b->blog_url;
    $page->BlogDescription = $b->blog_desc;
    
    // owner profile
    $owner = new User();
    $owner = user_load($b->blog_owner_id);
    $page->BlogOwnerProfileUrl = "http://". $_SERVER['HTTP_HOST'].WEB_ROOT. "/profile.php?username=$owner->username";
    
    
    $page->HomeLink = false; // for a link to the homepage; bcoz we dont need a link to the homepage on the homepage
    
    //$num_frontpage_posts = $b->num_frontpage_posts;
    ...
    for ( $i=0; $i<count($post_ids) ; $i++ ) { // for posts on the page
      ...
      $page->Blogger[$i]->BlogItemAuthorNickname = $author->name; //'Akky'; //using name instead of the nicknname
